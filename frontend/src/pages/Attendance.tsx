...
const AttendancePage: React.FC = () => {
  ...
  const fetchAttendance = async () => {
    setIsLoading(true)
    try {
      const records = user?.role === 'admin' ? await mockApi.getAttendance() : await mockApi.getAttendance(user?.employeeId)

      setAttendanceRecords(records)

      const today = new Date().toISOString().split('T')[0]
      const todayRecord = records.find((r) => r.date === today && r.employeeId === user?.employeeId)
      ...
      >
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
            ...
            <p className="text-2xl font-bold">{todayAttendance?.hoursWorked || '0'} hrs</p>
          </div>
        </div>
              ...
              <tbody>
                {attendanceRecords.map((record, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-sm font-medium text-gray-900">{formatTime(record.checkIn)}</td>
